...
export class Claude implements AIBase {
    ...
    translate(content: string, target: string): Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            My first sentence is "istanbulu cok seviyom burada olmak cok guzel"`).then((conversation: any) => {
                conversation.sendMessage(content).then((res: any) => {
