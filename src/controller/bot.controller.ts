    ...
    async postPing(@Body() msg: TGWenHook) {
        switch (msg.message.text) {
            ...
            case '/en':
                ...
                this.tg.sendMessage(msg.message.chat.id, 'Please input the Chinese or Japanese you want to translate');
                return { success: true, message: 'OK'};
            default:
                if (this.mapCache.has(msg.message.chat.id.toString())) {
                    const type = this.mapCache.get(msg.message.chat.id.toString());
                    const transaction_content = await this.transaction.getAnswer(msg.message.text, type);
